...
if(empty($username)){
    ...
    $aofficer = Database::search("SELECT * FROM `academic_officer` WHERE `email`='".$username."' AND `password`='".$password."' AND `verification_code`='".$vcode."'");
    $naofficer = $aofficer->num_rows;
    if($naofficer==1){
        $daofficer = $aofficer->fetch_assoc();
        if($daofficer['status_id']=="2"){
            Database::iud("UPDATE `academic_officer` SET `status_id`='1' WHERE `id`='".$daofficer['id']."'");
